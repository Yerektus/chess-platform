  ...
  return (
    <header className="sticky top-0 z-30 border-b border-[var(--color-border)] bg-[var(--color-bg)]/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-[1280px] items-center gap-6 overflow-x-auto px-4 sm:px-6 lg:px-8">
        <Link href="/" className="shrink-0 text-[15px] font-bold text-[var(--color-text-primary)]">
          Chess Platform
        </Link>

        <nav className="ml-auto flex min-w-max items-center gap-3 text-[13px] font-medium text-[var(--color-text-secondary)]">
          <ThemeToggle className="h-9" />
          <Button asChild className="h-9 rounded-[8px] border-[var(--color-accent)] px-4 text-[13px]" variant="ghost">
            <Link href={loginHref}>
              Log In
            </Link>
          </Button>
